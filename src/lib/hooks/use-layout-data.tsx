...
export const useFeaturedProductsQuery = (customLimit: number) => {
  const { cart } = useCart()

  const queryResults = useQuery(
    ["layout_featured_products", cart?.id, cart?.region, customLimit],
    () => fetchFeaturedProducts(cart?.id!, cart?.region!, customLimit),
    {
